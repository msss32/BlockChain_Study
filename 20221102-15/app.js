...
// block 클래스로 만들 블록들을 체인으로 연결시켜줄 chain을 클래스를 만들기
...
// 작업 증명 역사
// 1993년 모니 나노어가 작업증명의 기본 개념을 고안
// 1997년 영국의 암호학자인 애덤백의 해시캐시고 이후에
// 2009년 이 기술은 사토시 나카모토라는 사람이 비트코인에 적용이 되어 오늘날까지 사용

// 해시캐시는 대량으로 스팸메일을 방지하고자 고안된 것
// 이메일을 보내기 위해서 작업증명 알고리즘을 이용해서 해시값을 찾고
// 그 보상으로 발행 되는 우표 같은 것
// 이 과정이 시간과 비용 많이들어서
// 대량 발생되는 스팸메일을 막을수 있는 방법으로 이 방식을 사용

// 작업증명 방식을 거래가 발생하면 해당 거래가 유용한지에 대한 합의 검증 방식

// 간단히 설명

// 작업증명은 어려운 수학문제 푸는 것이라고 생각하고
// 채굴 하는데 작업 증명을 하는것이 채굴
// 어려운 수학 문제를 풀게해서 이 문제를 푼사람한테 보상으로 코인을 주는 것

// 여기서 특정 조건을 만족하는 논스값을 찾는 것

// 블록체인 상의 모든 참여자는 동일한 순서로 블록을 연결하기 위해서
// 합의 알고리즘이 필요하고
// 대표적인 합의 알고리즘은 POW, POS, DPOS, POA등이 있음
...
// 작업 증명 알고리즘은 Difficulty 조절 알고리즘을 이용해서 약 10분당 1개의 블록이 생성 되는것을 보장하게 됨
...
// 난이도 조절을 1 증가시키고 timeExpected * 2 보다 클 경우에는 난이도를 1 감소
