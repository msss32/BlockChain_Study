...
// 암호화폐의 지갑은 왜 쓸까 
// 본인의 자산을 암호화폐를 다른 누군가에게 상대방에게 양도 하기위해서
// 사용하는 소프트웨어의 한종류 라고 보면 된다.
...
// 공개키로는 네트워크 참여자의 거래내역의 사람들의 거래가 정상인지 아닌지를 확인 할수 있고
// 개인키는 개인키를 이용해서 직접 거래하고 개인키는 절때 잃어 버리면 안된다.
// 잃어 버리면 본인이 소유하고 있는 암호화폐가 묶여있다고 보면됨.

// 단방향 암호화를 통해 키를 만든다.
// 개인키의 역활은 공개키를 만들어 주고 만든 공개키로 주소를 만들어준다.
...
// 공개키의 역활은 암호화폐 전송 받을때 사용(계좌 번호) 그리고 암호화폐의 거래내역이
